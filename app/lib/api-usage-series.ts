...
function fillMissingDays(
  ...
  for (let i = 0; i < days; i++) {
    ...
    out.push(
      byKey.get(key) || { period: key, requests: 0, success: 0, uniqueIps: 0 }
    )
  }
  return out
}
...
/**
 * Volume + distinct IPs per UTC bucket for the given API keys.
 * Daily: last DAILY_CHART_DAYS UTC days (including zeros). Weekly: 12 ISO weeks. Monthly: 12 months.
 */
export async function getApiUsageTimeSeries(apiKeyIds: string[]): Promise<ApiUsageChartsData> {
  if (!apiKeyIds.length) {
  ...
  const [dailyAgg, weekRows, monthRows] = await Promise.all([
    prisma.$queryRawUnsafe<RawRow[]>(
      `SELECT
         ...
         COUNT(*) FILTER (WHERE status >= 200 AND status < 300)::bigint AS success,
         COUNT(DISTINCT ip)::bigint AS unique_ips
       FROM api_usage
       WHERE "apiKeyId" = ANY($1::text[])
      ...
      `WITH anchors AS (
       ...
       LEFT JOIN (
         SELECT
           ...
           COUNT(*)::bigint AS cnt,
           COUNT(*) FILTER (WHERE status >= 200 AND status < 300)::bigint AS succ,
           COUNT(DISTINCT ip)::bigint AS ips
         FROM api_usage
         WHERE "apiKeyId" = ANY($1::text[])
         GROUP BY 1
