...
export async function POST(
  ...
  try {
    ...
    // 6. Enforce device binding when key was issued for a specific machine
    const boundMachineId = auth.apiKey.machineId?.trim()
    if (boundMachineId && machineId && String(machineId) !== boundMachineId) {
      return NextResponse.json(
        {
          error: 'This API key is bound to another device. Request a new key via Discord (/key pair) on this PC.',
    ...
    // 7. Check for existing activation for THIS specific machine
