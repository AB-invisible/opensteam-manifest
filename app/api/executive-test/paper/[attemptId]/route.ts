...
/**
 * Candidate PDFs for their own Executive Officer exam attempts.
...
export async function GET(
  ...
  try {
    ...
    const tt = await prisma.trialTest.findFirst({
      where: { id: attemptId, userId: auth.data.dbUser.id, examKind: EXEC_OFFICER_EXAM_KIND },
      select: {
        id: true,
        status: true,
        submittedAt: true,
    ...
    const name =
      variant === "blank"
        ? `executive-officer-exam-blank-${tt.id.slice(0, 8)}.pdf`
        : `executive-officer-exam-your-answers-${tt.id.slice(0, 8)}.pdf`;
